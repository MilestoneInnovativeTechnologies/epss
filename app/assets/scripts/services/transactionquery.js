export const TransactionQueryBuilder = class {
    constructor(type){
        this.fieldMaps = { docno:'TR.`docno`',customer:'U.`name`',date:'TR.`date`',quantity:'SPT.`quantity`',nature:'PTN.`name`',total:'TD.`total`',amount:'TD.`amount`',tax:'TD.`tax`',discount:'TD.`discount`',store:'ST.`name`',product:'P.`name`',code:'P.`code`',rate:'PL.`price`',taxValue:'P.`taxfactor`',taxRate:'P.`taxfactor` * 100',direction:'SPT.`direction`',type:'PTT.`name`',cid:'TR.`customer`',pid:'SPT.`product`',sid:'SPT.`store`',id:'TR.`_ref`',eid:'E.`id`',executive:'E.`name`' };
