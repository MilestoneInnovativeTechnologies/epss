<template>
    <GridLayout :rows="rows">
        ...
        <StackLayout row="1" orientation="horizontal"><AppButton class="c-white btn-active m-l-0" v-bind="bindVars(btn)" :width="actWidth(btn)" v-for="(btn,idx) in actions" @tap.native="$emit(eventName(btn))" :key="['app',unique,'btn',idx].join('-')">{{ btn }}</AppButton></StackLayout>
    </GridLayout>
</template>
