...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters({ getProductsPerList:'Product/listProducts',getProductDetail:'Product/product',allProducts:'Product/_tableDataById' }),
            ...
            productDetails(){ return _.map(this.productIds,(pid) => (this.pReady) ? products[pid] : this.getProductDetail(pid)); },
            fewProductDetails(){ return _.map(this.productIds.slice(0,this.perPage*3),(pid) => (this.pReady) ? products[pid] : this.getProductDetail(pid)); },
        ...
        mounted(){
            if(!this.pReady) setTimeout(() => { products = this.allProducts('products'); this.pReady = true; },3000);
        }
    }
