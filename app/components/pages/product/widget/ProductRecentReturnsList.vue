    ...
    export default {
        ...
        computed: {
            ...
            recentReturnsSource(){ let vm = this; return _.mapValues(vm.rProdTrans,(rProdTran) => {
                ...
                let date = moment(rProdTran.date).format(__.DOCDATE_FORMAT), nature = _.get(vm.transNatures,rProdTran.nature),
                    customer = _.get(vm.users,rTran.customer), docno = rTran.docno, quantity =_.round(rProdTran.quantity,__.QUANTITY_DECIMAL);
                return { docno,customer,date,quantity,nature }
            }) }
        },
        methods: {
