...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            docno(){ return this.doc(this.store,this.final.fycode,this.fncode) },
        },
        methods: {
            setFinal(data){ this.final = Object.assign({},this.final,data) },
            selectProducts(){
                let master = { user:this.user,docno:this.docno,date:this.final.date,customer:this.id,fycode:this.final.fycode,fncode:this.fncode,payment_type:this.final.payment_type,_ref:this._ref() };
