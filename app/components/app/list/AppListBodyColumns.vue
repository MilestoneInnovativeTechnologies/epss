<template>
    <GridLayout class="applist-tbody-columns" :rows="rows" :columns="columns">
        <TextRegular v-for="(path,colNo) in mainLayoutPaths" row="0" :col="colNo" class="applist-tbody-column" :class="linkClass(path)" @tap.native="navigate(path)" :key="key(colNo)">{{ content(path) }}</TextRegular>
