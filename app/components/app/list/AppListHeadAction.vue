...
<script>
    export default {
        name: "AppListHeadAction",
        props: ['action','collection','items'],
        computed: {
            status(){ let collLength = this.collection.length, itemsLength = _.toSafeInteger(this.items); return (collLength) ? ((collLength === itemsLength) ? 1 : 2) : 0 }
        }
    }
</script>