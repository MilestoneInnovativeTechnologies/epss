...
let debounceEventSource = null, ESOnMessage = null, ESOnError = null;

export function init(ctx) {
    let func = _.bind(doEventSource,ctx);
    debounceEventSource = _.debounce(func,reConnectMinDelay * 1000,{ leading:true });
    ESOnMessage = _.bind(esOnMessage,ctx); ESOnError = _.bind(esOnError,ctx)
...
function doEventSource(url){
    ...
    eventSource.events.on('onMessage',data => ESOnMessage(data));
    eventSource.events.on('onError', data => ESOnError(data));
}
