import {
    ...
    finalize_failed_transfer
} from '../../mutation-types';

export default {
    [add_configuration_to_server_queue](state, { config,success,fail }) {  state.queue.push(_.assign(config,state.config,{ success,fail })); },
    [start_process_queue](state) { state.processing = state.queue.shift(); state.process_time = __.now ? __.now() : parseInt(new Date().getTime()/1000); state.success = state.processing.success; delete state.processing.success; state.fail = state.processing.fail; delete state.processing.fail },
