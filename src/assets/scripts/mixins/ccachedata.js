...
export const CCacheDataMixin = {
    ...
    methods: {
        CCacheDataRefill(table,key,method,args,get){
            timer.clearTimeout(this.CCacheDataTimeOut);
            if(!CCache.hasOwnProperty(table)) return this.CCacheDataTimeOut = timer.setTimeout(this.CCacheDataRefill,500,table,key,method,args,get);
