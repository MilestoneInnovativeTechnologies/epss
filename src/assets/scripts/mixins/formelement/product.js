...
export const FormElementMixinProduct = {
    computed: {
        feValuesProduct(){
            let list = CCache['products'].all(), key = 'id', label = 'narration', listArray = _.map(list,(product) => _.zipObject([key,label],[product[key],product[label] + ' ('+_.toNumber(product['stock'])+')']));
