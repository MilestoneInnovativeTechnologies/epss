...
export default {
    ...
    mutations:{
        ...
        [set_db_data](state, { db,data }) {
            if(!_.has(state.dbData,db)) Vue.set(state.dbData,db,[]);
            Vue.set(state.dbData,db,data);
        },
    ...
    getters: {
        _get(state) {
            return (key) => (Array.isArray(key)) ? _.pick(state,key) : (_.includes(key,'.') ? _.get(state,key) : state[key])
        },
