...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            printEnabled(){ return this.$store.getters['Settings/setting']('print') === 'Yes' }
        },
        methods: {
            ...
            reloadComp(){ this.$navigateTo(NewSalesOrder.default,{ props:this.TO_Get(['store','fycode','fncode','title']),backstackVisible:false }) },
            saveTransaction(){
                ...
                this.saveSalesOrderTransaction({ sales_order,sales_order_items })
                    .then(ref => this.printEnabled ? this.FnPrint({ _ref:ref }).then(() => this.reloadComp()) : this.reloadComp());
            },
        }
    }
</script>
