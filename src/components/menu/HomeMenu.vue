<template>
    <StackLayout class="m-t-12" v-if="content" :width="width" :key="'hm-lyt-'+uKey">
        ...
        <GridMenuSection class="m-b-15" v-for="(title,idx) in conditional" :key="['hmgs',2,idx].join('-')" :title="title" :items="conditional_items[idx]" :height="height" :space="spacing" v-show="conditional_items[idx].length" />
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('Menu',['avoid','content','sections','section_items','commons','common_items']), ...mapState('App',{ scrWidth:'width' }),
        ...
        methods: {
            ...
            populateMenuItems(res){
                ...
                _.forEach(res,(Obj,idx) => {
                    if(this.avoidable(Obj)) return true;
            ...
            avoidable(obj){
                return _.some(this.avoid,(values,key) => _.includes(values,obj[key]));
                let status = false;
                _.forEach(this.avoid,(values,key) => { if(values.includes(obj[key])) status = true })
                return status;
            },
